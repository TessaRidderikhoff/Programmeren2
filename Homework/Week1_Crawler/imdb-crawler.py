...
def scrape_top_250(soup):
    ...
    # loop through movie links on top 250 movie page 
    for link in soup.find_all('td', class_= "titleColumn"):
        url = 'https://www.imdb.com'+link.a.get('href')
        # add url of movie's page to list
...
def scrape_movie_page(dom):
    ...
    # add movie title
    for title in dom.find_all(class_="title_wrapper"):
        movinfo = title.h1.text
        
        # split movie title from year
        movtitle, movyear = movinfo.split("\xa0(")
        
        # add title to movie information list
        movie_info.append(movtitle)
        
        # add duration of movie to list
        movie_info.append(title.time.get_text(strip=True))

        # select genre(s) of movie
        genrestr = ""
        for genre in title.find_all('a', href=re.compile("genre")):
            genrestr += genre.text + "; "
        
        # remove last ;
        genrestr = re.sub("\; $", "", genrestr)
        
        # add genre(s) of movie to list
        movie_info.append(genrestr)

    # select director(s) of movie
    directorstr = ""
    for info in dom.find_all(itemprop = "director"):
        directorstr += info.a.get_text(strip=True) + "; "
    
    # remove last #
    directorstr = re.sub("\; $", "", directorstr)
    
    # add director(s) of movie to list
    movie_info.append(directorstr)

    # add writer(s) of movie to list
    writerstr = ""
    for writer in dom.find_all(itemprop="creator", itemtype = re.compile("Person")):
        writerstr += writer.a.text + "; "
    writerstr = re.sub("\; $", "", writerstr)
    movie_info.append(writerstr)

    # add actor(s) of movie to list
    actorstr = ""
    for actor in dom.find_all(itemprop = "actors"):
        actorstr += actor.a.get_text(strip=True) + "; "
    actorstr = re.sub("\; $", "", actorstr)
    movie_info.append(actorstr)
    
    # add rating of movie to list
    for rate in dom.find_all('span', itemprop = "ratingValue"):
        rating = rate.text
        movie_info.append(rating)

    # add number of votes for the rating to list
    for count in dom.find_all(itemprop = "ratingCount"):
        ratecount = count.text
        movie_info.append(ratecount)
